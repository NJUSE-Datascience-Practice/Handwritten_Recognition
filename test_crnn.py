...
class Test_CRNN(object):
    def __init__(self, batch_size=None):
        ...
        saver.restore(self.sess, "./model/93/ckpt")
...
if __name__ == "__main__":
    ...
    for root, dirs, files in os.walk(file):
        for name in files:
            a = Test_CRNN()
            print(os.path.join(name)+' '+str(a.test_img(os.path.join(root, name))))
            f.write(os.path.join(name) + ' ' + str(a.test_img(os.path.join(root, name))) + '\n')
    f.close()
